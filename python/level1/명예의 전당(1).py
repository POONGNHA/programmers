...
# 풀이유추
# 매일 하나씩 append 해서 sort한 값을 result list에 추가해서 [-1]을 return하면 쉽게 풀릴 문제

# 변수선언
# input : k, score :: int, list
# todayList : list
# result :: list
...
def solution(k, score):
    todayList = []
    result = []
    for score_idx in range(0,len(score)):
        todayList.append(score[score_idx])
        todayList.sort(reverse=True)
        if len(todayList) > k:
            todayList = todayList[:k]
        result.append(todayList[-1])
    return result
