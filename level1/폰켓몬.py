...
# 풀이 유추
# set으로 중복없게 만들어 원소종류가 몇개 되는지 가늠하고 count()로 원소의 갯수를 센다.
# 경우 1 : nums/2 가 len(set(nums))보다 작거나 같을때 : return nums/2
# 경우 2 : nums/2 가 len(set(nums))보다 클 때 : return len(set(nums)) * min(count(nums))

# 변수 선언
# input : nums : list
# numSet : set
# countNumsElement : hash
...
def solution(nums):
    ...
    countNumsElement = {}
    for i in numSet:
        countNumsElement[i] = nums.count(i)
