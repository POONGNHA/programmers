...
# 은지는 우박수열을 좌표 평면 위에 꺾은선 그래프로 나타내보려고 합니다. 초항이 K인 우박수열이 있다면, x = 0일때 y = K이고 다음 우박수는 x = 1에 표시합니다.
# 이런 식으로 우박수가 1이 될 때까지 점들을 찍고 인접한 점들끼리 직선으로 연결하면 다음과 같이 꺾은선 그래프를 만들 수 있습니다.
# 은지는 이렇게 만든 꺾은선 그래프를 정적분 해보고 싶어졌습니다. x에 대한 어떤 범위 [a, b]가 주어진다면 이 범위에 대한 정적분 결과는
# 꺾은선 그래프와 x = a, x = b, y = 0 으로 둘러 쌓인 공간의 면적과 같습니다. 은지는 이것을 우박수열 정적분이라고 정의하였고 다양한 구간에 대해서 우박수열 정적분을 해보려고 합니다.
...
# 풀이유추
# 넓이를 구하는거니까 x가 1씩 증가할때 dy/dx 즉 y의 증감량을 체크를 하면 됨
# input y가 짝수면 2로 나누고, 홀수면 *3 +1 을 해줌
# 그리고 input y가 짝수면 감소, 홀수면 증가하는 것을 알수 있음
# input y와 x+1이 되고나서의 y의 대소를 그것으로 판별하면 될듯
# 더 작은 y의 크기가 밑의 사각형의 넓이가 되고, 1/2 * y의 크기가 위의 삼각형 넓이가 됨
# y가 1이되면 거기서 종료
